...
//GCNv2 RGB-D
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imMask, const double &timeStamp, GCNextractor* extractor, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
            :mImMask(imMask), mpORBvocabulary(voc),mpGCNextractorLeft(extractor), mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
             mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),mImGray(imGray),mIsKeyFrame(false),mImDepth(imDepth)
{
    // Frame ID
    mnId=nNextId++;
...
//GCNv2 RGBD with output
    Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imMask, const cv::Mat &imRGB,
                 const double &timeStamp,  GCNextractor* extractor, ORBVocabulary* voc,
                 cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
            :mImMask(imMask), mpORBvocabulary(voc), mpGCNextractorLeft(extractor), mpORBextractorLeft(static_cast<ORBextractor*>(NULL)), mpORBextractorRight(static_cast<ORBextractor*>(NULL)), mImGray(imGray),
             mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mIsKeyFrame(false), mImDepth(imDepth), mImRGB(imRGB)
    {
        // Frame ID
        mnId=nNextId++;
