                                ...
                                cv::Mat &imDOut, cv::Mat &maskOut)
{
    ...
    if (getenv("NN_ONLY") != nullptr || getenv("FULL_RESOLUTION") == nullptr)//Change
    {
        ...
        cv::resize(imMask, imMask, cv::Size(320, 240));//Change
    ...
    {
        // GCN
        mCurrentFrame = Frame(mImGray,imDepth,imMask,_imRGB,timestamp,mpGCNextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);//Change
    }
    else
    ...
    {
        // GCN
        mCurrentFrame = Frame(mImGray,imDepth,imMask,imRGBOut,timestamp,mpGCNextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    }
    else
                                ...
                                const double &timestamp)
{
    ...
    {
        // GCN
        mCurrentFrame = Frame(mImGray,imDepth,imMask,timestamp,mpGCNextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);//Change
    }
    else
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
           ...
           //f(!bOK)//Change
           //{   cout<<"!bOK::bOK = TrackLocalMap();"<<endl;
        ...
        if(bOK)
        {
            ...
            mLastPose = mCurrentFrame.mTcw.clone();//Change
        }
