...
//GCN Stereo
             ...
             mpReferenceKF(static_cast<KeyFrame*>(NULL))
    {
        ...
        // EXtractGCN extraction
        cout<<"EXtractGCN extraction"<<endl;
        ...
        if(mvKeys.empty())
            return;
        cout<<"Frame::Frame::UndistortKeyPoints()"<<endl;
        UndistortKeyPoints();
        cout<<"Frame::Frame::ComputeStereoMatches();"<<endl;
        ComputeStereoMatches_GCN();
        ...
        {
            cout<<"ComputeImageBounds(imLeft);"<<endl;
        ...
        mb = mbf/fx;
        cout<<"Frame::Frame::AssignFeaturesToGrid();"<<endl;
...
//GCN Mono used
    Frame::Frame(const cv::Mat &imGray, const double &timeStamp, GCNextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
            :mpORBvocabulary(voc),mpGCNextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
             mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
    {
        // Frame ID
        mnId=nNextId++;
...
//Change
void Frame::ComputeStereoMatches_GCN() {
    ...
    const int NR  = mvKeysRight.size();
        //cout<<"NR  = mvKeysRight.size() : "<<NR<<endl;
    ...
    for(int iL=0; iL<N; iL++) {
        ...
        //cout<<"const cv::Mat &dL = mDescriptors.row(iL);"<<endl;
        ...
        //cout<<" for(size_t iC=0; iC<vCandidates.size(); iC++)"<<endl;
        ...
        if(bestDist<thOrbDist)
        {
            ...
            //cout<<"IL = mpGCNextractorLeft->GetImage()"<<endl;
            cv::Mat IL = mpGCNextractorLeft->mvImagePyramid[0].rowRange(scaledvL-w,scaledvL+w+1).colRange(scaleduL-w,scaleduL+w+1);
            //cout<<IL.type()<<endl;
            //cout<<"IL.convertTo(IL,CV_32F);"<<endl;
            IL.convertTo(IL,CV_32F);
            cout<<"IL - IL.at<float>(w,w)"<<endl;
            IL = IL - IL.at<float>(w,w) * cv::Mat::ones(IL.rows,IL.cols,CV_32F);//窗口中的每个元素减去正中心的那个元素，简单归一化，减小光照强度影响
            //IL = IL - IL.at<float>(w,w) * cv::Mat::ones(IL.rows,IL.cols);
            //cout<<"int bestDist = INT_MAX;"<<endl;
            int bestDist = INT_MAX;
            int bestincR = 0;
            const int L = 5;
            cout<<"vector<float> vDists;"<<endl;
            vector<float> vDists;
            vDists.resize(2*L+1); // 11
            //cout<<"vDists.resize(2*L+1);"<<endl;
            // 滑动窗口的滑动范围为（-L, L）,提前判断滑动窗口滑动过程中是否会越界
            const float iniu = scaleduR0-L-w; //这个地方是否应该是scaleduR0-L-w (wubo???)
            const float endu = scaleduR0+L+w+1;
            if(iniu<0 || endu >= mpGCNextractorRight->mvImagePyramid[0].cols)
                continue;
            cout<<"for(int incR=-L; incR<=+L; incR++)"<<endl;
            for(int incR=-L; incR<=+L; incR++)
            {
                // 横向滑动窗口
                //cout<<"cols: "<<mpGCNextractorRight->mvImagePyramid[0].cols<<endl;
                //cout<<"scaleduR0+incR-w "<<(scaleduR0+incR-w)<<endl;
               // cout<<"scaleduR0+incR+w+1 "<<(scaleduR0+incR+w+1)<<endl;
                cv::Mat IR = mpGCNextractorRight->mvImagePyramid[0].rowRange(scaledvL-w,scaledvL+w+1).colRange(scaleduR0+incR-w,scaleduR0+incR+w+1);
            ...
            float bestuR = 1*((float)scaleduR0+(float)bestincR+deltaR);

            //float bestuR = mvKeysRight[bestIdxR].pt.x;
